...
- `Shaders/S_ProceduralMoonSurface.shader`/`Materials/M_PlanetCrateredMoon.mat` -
  opaque URP-lit, texture-free planet surface shader. Object-space 3D value
  noise creates seam-free large lunar mottling, fine dusty breakup, and
  matching analytic bump normals without depending on source mesh UVs;
  supplies forward/shadow/depth/depth-normal passes. The material preserves
  the warm clay/ochre base color and low smoothness with tonal/roughness/
  normal variation.
...
- `Shaders/S_ProceduralSpaceSkybox.shader`/`Materials/M_ProceduralSpaceSkybox.mat` -
  ...
  world direction); the material is SampleScene's configured skybox.
...
- `Models/Characters/Astronaut_FinnTheFrog.fbx` - rigged playable-character
  ...
  stays cosmetic. `PlayerRig.prefab` nests this asset.
- `Materials/M_Astronaut.mat` - URP Lit material binding `T_SpacePalette`
  directly (unlike `M_Ground`, this mesh's UVs sample the atlas rather than
  a flat sampled color).
